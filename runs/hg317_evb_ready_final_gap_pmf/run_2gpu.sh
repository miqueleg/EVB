...
run_root="runs/hg317_evb_ready_final_gap_pmf"
...
run_pair() {
  local rep_a="$1"
  local rep_b="$2"
  run_replica "$run_root/configs/${rep_a}.yaml" 0 "outputs/hg317_evb_ready_final_gap_pmf/${rep_a}" &
  pid0=$!
  run_replica "$run_root/configs/${rep_b}.yaml" 1 "outputs/hg317_evb_ready_final_gap_pmf/${rep_b}" &
  pid1=$!
  wait "$pid0"
  wait "$pid1"
}
...
echo "[$(date -Is)] HG3.17 EVB-ready final gap PMF pipeline complete"
