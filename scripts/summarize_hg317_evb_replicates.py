...
def main() -> None:
    ...
    outputs_root = repo_root / "outputs" / "hg317_evb_gap_umbrella_replicates"
    replicate_dirs = sorted(
        path for path in outputs_root.glob("rep*") if (path / "analysis" / "pmf_gap.csv").is_file()
    )
    if not replicate_dirs:
        raise SystemExit(f"No analyzed replicate PMFs found under {outputs_root}")
    ...
    print(f"Summarized {len(replicate_dirs)} replicates into {destination}")
